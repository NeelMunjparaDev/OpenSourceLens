...
def get_repo_health_score(spark, input_table, top_n=50):
    """
...
def get_branch_activity_distribution(spark, input_table, top_n=100):
    """
...
def get_contributor_consistency(spark, input_table, top_n=20, min_active_days=1):
    """
...
def write_to_gold_table_y_m_d(df: DataFrame, table_name: str, create: bool = False):
    """
...
def write_to_gold_table_y_m(df: DataFrame, table_name: str, create: bool = False):
    """
    Write the transformed DataFrame to the gold table.
    """
    writer = df.writeTo(table_name).using("iceberg")
